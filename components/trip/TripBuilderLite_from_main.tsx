...
/**
 * TripBuilderReceipt
 * Production-lean, deterministic ΓÇ£receipt themeΓÇ¥ chat that only offers curated answers.
...
const TripBuilderReceipt: React.FC<TripBuilderReceiptProps> = ({
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      const savingToastId = toast.loading("Saving trip requestΓÇª", {
        duration: Infinity,
      });
      ...
      toast.error("We couldnΓÇÖt save your request. Try again.", {
        action: {
          label: "Retry",
  ...
  return (
    <div className={className}>
      {submissionState === "saving" && (
        <FlightLoader fullscreen message="Submitting your trip requestΓÇª" />
      )}
      {!isTorn && (
        <>
          <div
            ...
            >
              <div className="header">
                <span className="brand-mini">
                  {title || "TRIP BUILDER ΓÇó RECEIPT"}
                </span>
                <div className="rowy">
                  {onExitToForm && (
              ...
              >
                {!oneAtATime && chatHiddenCount > 0 && (
                  <div className="line faded">
                    <span>ΓÇª {chatHiddenCount} earlier lines hidden</span>
                    <button
      ...
      <style jsx>{`
        ...
        /* A tiny ΓÇ£paper mouthΓÇ¥ at the top for vibe */
        .paper::before {
          content: "";
          display: block;
          height: 6px;
