...
/**
 * TripBuilderReceipt
 * Production-lean, deterministic Gǣreceipt themeGǥ chat that only offers curated answers.
...
const TripBuilderLite: React.FC<TripBuilderReceiptProps> = ({
  seed,
  onSubmit,
  onExitToForm,
  persistKey,
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      const savingToastId = toast.loading("Saving trip requestGǪ", {
        duration: Infinity,
      });
      ...
      // Save cart activities to the trip request
      if (createdId && cartActivities.length > 0) {
        console.log("[TripBuilder] Saving cart activities", {
          tripRequestId: createdId,
          activityCount: cartActivities.length,
        });
        
        const activityPromises = cartActivities.map(async (activity) => {
          try {
            const activityResponse = await fetch(
              `/api/trip-requests/${createdId}/activities`,
              {
                method: "POST",
                headers: { "Content-Type": "application/json" },
            ...
            if (!activityResponse.ok) {
              console.warn(`[TripBuilder] Failed to save activity ${activity.id}`);
            }
            
            return activityResponse;
          } catch (err) {
            console.error(`[TripBuilder] Error saving activity ${activity.id}`, err);
            return null;
          }
        });
        
        await Promise.allSettled(activityPromises);
        console.log("[TripBuilder] Cart activities saved");
      }
      
      // Always clear cart after successful submission, regardless of whether activities were saved
      clearCart();
      console.log("[TripBuilder] Cart cleared after successful submission");
      ...
      toast.error("We couldnG��t save your request. Try again.", {
        action: {
          label: "Retry",
  ...
  return (
    <div className={`${className} min-h-screen bg-white`}>
      {submissionState === "saving" && (
        <FlightLoader fullscreen message="Submitting your trip requestGǪ" />
      )}
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-6 lg:py-12">
        ...
        <div className="mb-8 flex items-start gap-4">
          <button
            onClick={() => router.back()}
            className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-slate-200 bg-white transition-all hover:border-slate-300 hover:shadow-sm"
          >
          ...
          <div className="flex-1 min-w-0">
            <h1 className="text-2xl lg:text-3xl font-bold text-slate-900">Complete Your Journey</h1>
            <p className="text-sm text-slate-600 mt-1">{cartActivities.length} experience{cartActivities.length !== 1 ? 's' : ''} selected</p>
          </div>
          
          {/* Progress Steps */}
          <div className="hidden lg:flex items-center gap-3 shrink-0">
            <div className="flex items-center gap-2 rounded-full bg-slate-900 px-4 py-2.5">
              <div className="flex h-5 w-5 items-center justify-center rounded-full bg-white text-xs font-bold text-slate-900">
                ✓
              </div>
              <span className="text-sm font-semibold text-white">Activities</span>
            </div>
            <div className="h-0.5 w-8 bg-slate-200"></div>
            <div className="flex items-center gap-2 rounded-full bg-blue-600 px-4 py-2.5">
              <div className="flex h-5 w-5 items-center justify-center rounded-full bg-white text-xs font-bold text-blue-600">
                2
              </div>
  ...
  <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-center">
          ...
          {/* Left Column: Your Itinerary / Selected Activities */}
          <div className="lg:col-span-4 order-2 lg:order-1">
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 lg:sticky lg:top-6ax-w-sm">
              <h2 className="text-2xl lg:text-3xl font-bold leading-tight text-slate-900">Your Itinerary</h2>
              <p className="text-sm text-slate-600 mt-1 mb-3">
                {cartActivities.length} unique experience{cartActivities.length !== 1 ? 's' : ''} selected
              </p>
              ...
              {cartActivities.length > 0 ? (
                <div className="space-y-2">
                  {cartActivities.map((activity, index) => (
                    <div
                      key={activity.id}
                      className="flex gap-2.5 rounded-lg border border-slate-200 bg-white p-3 transition-all hover:shadow-sm"
                    >
                      <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-slate-900 text-xs font-bold text-white">
                        {index + 1}
                      </div>
                      <div className="flex-1 min-w-0">
                        <h3 className="font-semibold text-slate-900 text-xs leading-tight">{activity.name}</h3>
                  ...
                  <div className="mt-3 rounded-lg border border-slate-200 bg-white p-3">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-xl font-bold text-slate-900">
                        AED {cartActivities.reduce((sum, a) => sum + (a.price || 0), 0).toLocaleString()}
                      </span>
                    </div>
                ...
                <div className="rounded-lg border-2 border-dashed border-slate-200 bg-white p-6 text-center">
                  ...
                  <p className="text-xs font-medium text-slate-900">No activities selected yet</p>
                  <p className="mt-0.5 text-[10px] text-slate-500">Browse activities to add to your itinerary</p>
                </div>
              )}
                ...
                <div className="mt-3 rounded-lg bg-amber-50 border border-amber-200 p-2.5">
                  <div className="flex gap-2">
                    ...
                    <div className="flex-1">
                      <p className="text-[10px] font-semibold text-amber-900">Note</p>
                      <p className="text-[10px] text-amber-800 mt-0.5 leading-snug">
                        This estimate and itinerary reflect activities only.
                      </p>
                    </div>
                  </div>
                </div>
          ...
          {/* Right Column: Trip Builder Form */}
      ...
      {!isTorn && (
        ...
        >
          <div className="flex items-center justify-between gap-3">
            ...
            {/* Prefill button moved to bottom-right as a floating control */}
          </div>
          {/* Floating prefill button (bottom-right) */}
          {DEBUG_PREFILL_ENABLED && (
            <div className="fixed bottom-6 right-6 z-50">
              <button
                type="button"
                className="rounded-lg bg-slate-100 border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-200 shadow"
                ...
                >
                  ...
                  {submissionState === "saved" && (
                    <>
                      ...
                      <p className="mt-3 text-sm text-slate-600">
                        Hang tightG��your boarding pass and trip add-ons are loading.
                      </p>
                    </>
                  )}
                  {submissionState === "error" && (
                    <>
                      <h3 className="text-xl font-semibold text-slate-900">
                        We couldnG��t submit your request
                      </h3>
                      <p className="mt-3 text-sm text-slate-600">
              ...
              >
                {!oneAtATime && chatHiddenCount > 0 && (
                  <div className="line faded">
                    <span>GǪ {chatHiddenCount} earlier lines hidden</span>
                    <button
                      ...
                      >
                        ...
                        {isDesktop && !isDateRangeInvalid && (
                          <span className="hidden md:inline-flex items-center text-xs font-semibold text-slate-500" aria-hidden>
                            press <span className="ml-1 font-black text-slate-900">Enter</span> G�
                          </span>
                        )}
                        {isDateRangeInvalid && (
      ...
      <style jsx>{`
        .receipt {
          ...
          min-height: 60vh;
          --tb-step-label: clamp(11px, 0.6vw + 10px, 13px);
          --tb-question: clamp(20px, 2vw + 16px, 28px);
          --tb-answer: clamp(14px, 1vw + 13px, 16px);
          --tb-helper: clamp(12px, 0.4vw + 11px, 14px);
          --tb-meta: clamp(10px, 0.3vw + 9px, 12px);
          --tb-line-height-tight: 1.25;
          --tb-line-height-body: 1.5;
        }
        /* Tear animation */
        .receipt.tearing {
        ...
        /* A tiny Gǣpaper mouthGǥ at the top for vibe */
        .paper::before {
          content: "";
          display: block;
          height: 6px;
...
export default TripBuilderLite;
