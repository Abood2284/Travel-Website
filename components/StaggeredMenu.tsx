...
export interface StaggeredMenuProps {
  ...
  itemFontSize?: string; // e.g. 'clamp(1.5rem,4.5vw,2rem)'
  onMenuOpen?: () => void;
  onMenuClose?: () => void;
}
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  accentColor = "#5227FF",
  panelTheme = "light",
  itemFontSize = "clamp(1.75rem,5.5vw,2.5rem)",
  onMenuOpen,
  onMenuClose,
}: StaggeredMenuProps) => {
    ...
    <div className="sm-scope w-full h-full">
      ...
      >
        ...
        >
          ...
          >
            {logoText ? (
              <span className="sm-logo-text block text-base font-semibold tracking-[0.15em] uppercase text-current">
                {logoText}
              </span>
            ) : (
              <img
        ...
        <aside
          ...
          ref={panelRef}
          className={`staggered-menu-panel absolute top-0 right-0 h-full ${
            panelTheme === "dark" ? "bg-[#0b0b0b]" : "bg-white"
          } flex flex-col p-[6em_2em_2em_2em] overflow-y-auto z-10 backdrop-blur-[12px]`}
            ...
            >
              {items && items.length ? (
                items.map((it, idx) => (
                  ...
                  >
                    <a
                      className={`sm-panel-item relative ${
                        panelTheme === "dark" ? "text-white" : "text-black"
                      } font-semibold cursor-pointer leading-tight tracking-[-2px] uppercase transition-[background,color] duration-150 ease-linear block w-full no-underline pr-[1.4em] whitespace-normal break-words`}
                      style={{ fontSize: itemFontSize }}
                      href={it.link}
                      aria-label={it.ariaLabel}
                      data-index={idx + 1}
                    >
                      <span className="sm-panel-itemLabel block [transform-origin:50%_100%] will-change-transform">
                        {(() => {
                          const parts = it.label.trim().split(/\s+/);
                          if (parts.length === 2) {
                            return (
                              <>
                  ...
                  {socialItems.map((s, i) => (
                    <li key={s.label + i} className="sm-socials-item">
                      <a
                        ...
                        className={`sm-socials-link text-[1.2rem] font-medium ${
                          panelTheme === "dark"
                            ? "text-[#e5e5e5]"
                            : "text-[#111]"
                        } no-underline relative inline-block py-[2px] transition-[color,opacity] duration-300 ease-linear`}
                      >
                        {s.label}
                      </a>
                    </li>
